...
from data.models import CreateFolderRequest, DeleteResourceRequest, UploadLinkRequest
...
def folder_for_create(ya_api):
    ...
    yield folder_name
    ya_api.delete_resource(DeleteResourceRequest(path=folder_name, permanently=True))


@pytest.fixture
def folder_for_delete(ya_api) -> str:
    folder_name = make_unique_folder_name()
    ya_api.create_folder(CreateFolderRequest(path=folder_name))
    yield folder_name
    ya_api.clear_trash()
...
def folder_for_restore(ya_api) -> str:
    folder_name = make_unique_folder_name()
    ya_api.create_folder(CreateFolderRequest(path=folder_name))
    ya_api.delete_resource(DeleteResourceRequest(path=folder_name))
...
def upload_setup(ya_api, tmp_path):
    ...
    ya_api.create_folder(CreateFolderRequest(path=FileData.INPUT_FOLDER))
    ya_api.create_folder(CreateFolderRequest(path=FileData.OUTPUT_FOLDER))

    local_file_path = tmp_path / FileData.FILE_NAME
    yield str(local_file_path)

    ya_api.delete_resource(DeleteResourceRequest(path=FileData.INPUT_FOLDER, permanently=True))
    ya_api.delete_resource(DeleteResourceRequest(path=FileData.OUTPUT_FOLDER, permanently=True))


@pytest.fixture
def download_setup(ya_api, tmp_path):

    ya_api.create_folder(CreateFolderRequest(path=FileData.SDET_FOLDER))
    ...
    upload_link_status, upload_link_body = parse_response(
        ya_api.get_upload_link_response(UploadLinkRequest(path=FileData.SDET_FILE_PATH))
    )
    assert upload_link_status == 200, "Ожидался статус 200 при получении ссылки на загрузку"

    resp = ya_api.upload_file(upload_link_body["href"], local_file_path)
    assert resp.status_code == 201, "Ожидался статус 201 при загрузке файла"

    yield local_file_path

    ya_api.delete_resource(DeleteResourceRequest(path=FileData.SDET_FOLDER, permanently=True))
